...
class DistributionEstimationMutator(Mutator):
    def mutate(
        self, population: Population, unit: UnitOfEvolution, **kwargs
    ) -> UnitOfEvolution:
        # Only mutate the task using all the task prompts in the unit population
        mutant_population = [
            self.task_prompt_factory(
                self.run(
                    {
...
class Hypermutation(Mutator):
    ...
    def _call(self, inputs, run_manager=None, **kwargs):
        ...
        # population
        meta_mutator = self.mutate_mutator_chain.run(
